...
class _StealthProperty:
    """Returns the value in the instance, raises AttributeError on the class.

    if AttributeError is raised, python will automatically use __getattr__ to
...
def break_noisily_on_pickle(self):
    """Raises an exception when trying to pickle the class.

    If module name was not able to be determined using the functional syntax,
    this function will be stored on that class as __reduce__.  It's only
    purpose is to move the pickle error from /un/pickling to pickling.
...
class _EnumDict(dict):
    ...
    def __setitem__(self, key, value):
        ...
        if dunder(key) or hasattr(value, '__get__'):
